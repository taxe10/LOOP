...
data(pos0,:) = [];      %Consider pixels inside the region of interest
%GramSchmidt dimension reduction
e = GramSchmidt_avg_angle_corrected(data,dim).';
sum_data = e\data.';
data = sum_data.';
n_x = length(xrange);
n_y = length(yrange);
out = expectation_maximization_complex(data,k);
mu = out.a;
sigma = out.b;
p = out.c;
gamma = out.d;
...
for i=1:k
    ...
    plot(X,Y,'linewidth',2,'Color',[0,1, 0],'LineStyle','--')
    legend([num2str(dim),'D EM '],'Location','southeast')
    hold on
...
x = xrange;
y = yrange;
%Asumming the availability of pathology matrix
h=figure(4);
for i=1:k
    subplot(k,2,2*i-1)
